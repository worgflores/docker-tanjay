...
CREATE TABLE `market_account` (
  ...
  `unitno` varchar(100) DEFAULT NULL,
  `areasqm` decimal(16,4) DEFAULT NULL,
  `primaryunitid` varchar(50) DEFAULT NULL,
  ...
  `lastdatecovered` date DEFAULT NULL,
  `extrate` decimal(18,2) DEFAULT NULL,
  `extarea` decimal(18,2) DEFAULT NULL,
...
CREATE TABLE `market_account_recurringfee` (
  ...
  `partialbalance` decimal(16,4) DEFAULT NULL,
  PRIMARY KEY (`objid`),
  UNIQUE KEY `uix_acctid_txntype` (`acctid`,`txntype_objid`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*Table structure for table `market_account_unit` */

DROP TABLE IF EXISTS `market_account_unit`;
...
CREATE TABLE `market_rentalunit` (
  ...
  UNIQUE KEY `uix_market_rentalunit_section_number` (`indexno`,`sectionid`),
